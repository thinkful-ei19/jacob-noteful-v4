...
describe.only('Noteful API - Folders', function () {
  ...
  describe('GET /api/folders', function () {
    ...
    it('should return the correct number of folders', function () {
      ...
      const apiPromise = chai.request(app).get('/api/folders').set('Authorization', `Bearer ${token}`);
  ...
  describe('GET /api/folders/:id', function () {
    ...
    it('should respond with a 400 for an invalid ID', function () {
      ...
      return chai.request(app)
        .get(`/api/folders/${badId}`).set('Authorization', `Bearer ${token}`)
    ...
    it('should return an error when given a duplicate name', function () {
      ...
      return Folder.find({ userId: user.id }).select('id name').limit(2)
        .then(results => {
          ...
          return chai.request(app).put(`/api/folders/${item1.id}`)
            .set('Authorization', `Bearer ${token}`)
            .send(item1);
        })
        .catch(err => err.response)
